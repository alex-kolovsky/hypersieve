...
fn main() {
    ...
    // Create a file for a vector extension support.
    let vector_extension_support_file_path_raw =
        std::path::Path::new(&out_dir).join("vector_extension.rs");
    let mut vector_extension_support_file_path =
        std::fs::File::create(&vector_extension_support_file_path_raw).unwrap();
...
extern \"C\" fn save_vector_registers(vcpu: *mut Vcpu) {{
    naked_asm!(
        ...
        vlen_size = const crate::VLEN_SIZE,
    );
}}"
        )
        .unwrap();
...
pub const VLEN_SIZE: usize = {vlen_size};
